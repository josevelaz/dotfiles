...
If you are controlling herdr from inside an agent/tool subprocess, do not blindly split the currently focused pane from `herdr pane list`: focus may have moved to a different pane while the agent is still running. Prefer `HERDR_PANE_ID` when it is set; otherwise read `pane list` and use the focused pane.

```bash
TARGET_PANE="${HERDR_PANE_ID:-1-2}"
herdr pane split "$TARGET_PANE" --direction right --no-focus
```
...
### cwd behavior for new tabs and panes

- Herdr has a config knob at `[terminal].new_cwd` in `~/.config/herdr/config.toml`.
- `new_cwd = "follow"` is the setting to make new tabs and panes inherit context instead of starting in `$HOME`.
- With `follow`, new tabs inherit the workspace identity cwd.
- With `follow`, pane splits inherit the target/focused pane cwd.
- `new_cwd = "home"` forces new panes/tabs/workspaces to start in `$HOME` when no explicit `--cwd` is passed.
- Important limitation: there is no separate config-only setting to force every split pane to start at the workspace root regardless of the focused pane's cwd. If you need that behavior, use an explicit `--cwd` in CLI/API calls or custom wrapper commands/keybindings.
