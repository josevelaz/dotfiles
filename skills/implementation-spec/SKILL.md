...
- Before writing the spec file, verify the live workspace/path visible to the file tools. Do not rely only on session metadata, startup workspace snapshots, or remembered repo roots when choosing where to save the spec.
- If the expected project path is unavailable or the live backend cwd/path disagrees with the conversational context, stop and resolve the target location before writing. A spec saved to the wrong mounted repo is a failure, even if the document itself is good.
