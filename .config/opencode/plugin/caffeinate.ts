...
const AUTO_START_DISABLED_VALUES = new Set(["0", "false", "off", "no", "disabled"])
...
const STATUS_RUNNING_LINES = [
	(pid) => `Power status: coffee IV connected. Caffeinate is buzzing (pid ${pid}).`,
	(pid) => `Power status: full bean overload. Caffeinate is sprinting laps (pid ${pid}).`,
	(pid) => `Power status: jitter protocol engaged. Caffeinate is wide awake (pid ${pid}).`,
	(pid) => `Power status: triple shot deployed. Caffeinate has the wheel (pid ${pid}).`,
]
...
const ALREADY_RUNNING_CRASH_OUT_LINES = [
	(pid) => `I am already overcaffeinated (pid ${pid}). Please run crash_out before another cup.`,
	(pid) => `Still sipping from the same espresso (pid ${pid}). Run crash_out first.`,
	(pid) => `Coffee levels are maxed out already (pid ${pid}). Hit crash_out before refilling.`,
]

const ALREADY_RUNNING_AUTO_LINES = [
	(pid) => `Auto mode check-in: caffeine stream is already flowing (pid ${pid}).`,
...
const STARTED_LINES = [
	(pid) => `Brewed and injected pure espresso. Caffeinate is live (pid ${pid}).`,
	(pid) => `Coffee protocol online. Caffeinate is now running (pid ${pid}).`,
	(pid) => `Fresh roast activated. Display-sleep ban is in effect (pid ${pid}).`,
]
...
function parseLock(content) {
	const [version, pidRaw, startedAtRaw, sessionID, ownerPidRaw] = content.trim().split("|")
	...
	if (ownerPid !== undefined && (!Number.isInteger(ownerPid) || ownerPid <= 0)) return null
			...
			if (!isLockOwnedByCurrentProcess(lock)) {
				...
				if (requireCrashOut) {
					return {
						ok: false,
						pid: lock.pid,
						message: randomLineWithArg(ALREADY_RUNNING_CRASH_OUT_LINES, lock.pid),
					}
				}
...
export const CaffeinatePlugin = async () => {
	return {
		...
		tool: {
			...
			crash_out: tool({
				description: "Kill the session caffeinate process and remove the lockfile.",
				args: {},
				async execute(_args, context) {
