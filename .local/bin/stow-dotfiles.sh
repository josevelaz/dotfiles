...
# stowed symlinked parent (e.g. ~/.local → ~/dotfiles/.local). Use physical paths
# so ~/.local/bin/stow-dotfiles.sh resolves back to ~/dotfiles/.local/bin.
...
while [[ -L "$_script_path" ]]; do
	...
	if [[ "$_link_target" = /* ]]; then
		...
		_script_path="$(cd -P "$(dirname "$_script_path")" && pwd -P)/$_link_target"
	fi
done
SCRIPT_DIR="$(cd -P "$(dirname "$_script_path")" && pwd -P)"
DOTFILES_DIR="$(cd -P "$SCRIPT_DIR/../.." && pwd -P)"
